...
public class MeituanServiceImpl implements IMeituanService {
			...
			for (int j = 0; j < searchList.size(); j++) {
				try {
					...
					if(inFoList!=null&&inFoList.size()>0) {
						...
						boolean isConnet=ConnUtil.isHostConnectable(map.get("host"), Integer.valueOf(map.get("port")));
						if(!isConnet) {
							map=proxyInfoService.iGetProxyInFo();
						}
	...
	public KhShopInFo selectMtInFo(String poi,SearchCondition condition,Map<String,String> map) throws Exception {
		KhShopInFo info = new KhShopInFo();
